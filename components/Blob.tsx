...
export default function Blob() {
	...
	useEffect(() => {
		...
		const animate = () => {
			cur1.current.x += (mouse.current.x - cur1.current.x) * 0.005
			cur1.current.y += (mouse.current.y - cur1.current.y) * 0.005
			cur2.current.x += (mouse.current.x - cur2.current.x) * 0.010
			cur2.current.y += (mouse.current.y - cur2.current.y) * 0.010
			cur3.current.x += (mouse.current.x - cur3.current.x) * 0.016
			cur3.current.y += (mouse.current.y - cur3.current.y) * 0.016
		...
		<>
			<div ref={layer1} style={{ ...base, width: "700px", height: "700px", marginLeft: "-350px", marginTop: "-350px" }}>
				<div style={{
					...
					background: "radial-gradient(circle, rgba(196,176,250,0.4) 0%, rgba(180,196,255,0.2) 50%, transparent 75%)",
					filter: "blur(60px)",
				}} />
			</div>
			<div ref={layer2} style={{ ...base, width: "500px", height: "500px", marginLeft: "-320px", marginTop: "-200px" }}>
				<div style={{
					width: "100%", height: "100%", borderRadius: "50%",
					background: "radial-gradient(circle, rgba(245,168,208,0.35) 0%, rgba(224,168,245,0.18) 55%, transparent 78%)",
					filter: "blur(55px)",
				}} />
			</div>
			<div ref={layer3} style={{ ...base, width: "380px", height: "380px", marginLeft: "-100px", marginTop: "-280px" }}>
				<div style={{
					width: "100%", height: "100%", borderRadius: "50%",
					background: "radial-gradient(circle, rgba(255,191,168,0.32) 0%, rgba(245,168,192,0.15) 55%, transparent 78%)",
					filter: "blur(50px)",
				}} />
			</div>
		</>
